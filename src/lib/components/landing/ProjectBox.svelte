...
{#if project.pro !== "graphic designs"}
  ...
  {#if clicked}
    <section
      in:slide={{ axis: "x" }}
      out:slide={{ axis: "x" }}
      class="bg-[#000000b0] backdrop-blur-[34px] h-screen w-screen fixed left-0 top-0 z-[300] grid place-content-center text-white"
    >
      <div class="relative w-full h-full overflow-y-scroll p-5">
        ...
        >
        <h1
          class=" font-bold text-5xl w-[90%] md:w-[80%] m-auto uppercase whitespace-normal"
        >
          {project.pro}
        </h1>
        <div
          class="relative w-[90%] md:w-[80%] flex flex-wrap justify-between m-auto gap-4"
        >
          {#each project.projects as pro, i}
            <a href={pro.url}>
              <div class="  h-auto grid justify-between rounded-sm">
                <h2 class="font-bold text-2xl capitalize">{pro.name}</h2>
                <p class=" whitespace-normal">
                  {pro.details}
                </p>
                <div
                  class="rounded-lg bg-gray-900 h-[150px] mt-3 grid place-content-center text-2xl"
                >
                  <svg
